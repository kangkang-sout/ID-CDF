...
class PosLinear(nn.Linear):
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        weight = 2 * F.softplus(1 * torch.neg(self.weight)) + self.weight
...
class IDCD(nn.Module):
    def __init__(self, n_user: int, n_item: int, n_know: int, \
        ...
        self.ncd = nn.Sequential(
            OrderedDict([
                ('pred_layer_1', nn.Linear(user_dim, 128)),
                ('pred_activate_1', nn.ReLU()),
                ('pred_dropout_1', nn.Dropout(p=0.3)),

                ('pred_layer_2', nn.Linear(128, 64)),
                ('pred_activate_2', nn.ReLU()),
                ('pred_dropout_2', nn.Dropout(p=0.3)),

                ('pred_layer_3', nn.Linear(64, 1)),
                ('pred_activate_3', nn.Tanh()),
            ])
        ).to(device)
